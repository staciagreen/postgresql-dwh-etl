...
# В pgAdmin можно:
# 1. Подключиться к PostgreSQL серверу (host: db, port: 5432)
# 2. Исследовать таблицы и схемы
# 3. Запускать SQL запросы через Query Tool
# 4. Просматривать размеры таблиц и индексов
```
...
+-- docker/
|   +-- init/
|   |   +-- 00_full_init.sql     (альтернативная полная инициализация)
|   |
|   +-- seeder/
|       +-- Dockerfile           (Python 3.12 образ)
|       +-- requirements.txt     (зависимости: psycopg2, Faker)
|       +-- seed.py              (скрипт генерации данных)
|
+-- sql/                         (SQL скрипты в порядке выполнения)
|   +-- 10_schema.sql            (базовые схемы филиалов)
|   +-- 11_fk.sql                (внешние ключи филиалов)
|   +-- 20_dwh_schema.sql        (DWH: измерения и факты)
|   +-- 21_fdw.sql               (настройка распределённого доступа)
|   +-- 22_dwh_load.sql          (начальная загрузка DWH)
|   +-- 30_dm_schema.sql         (витрины данных)
|   +-- 31_dm_load.sql           (загрузка витрин)
|   +-- 31_dm_load_range.sql     (инкрементная загрузка витрин)
|   +-- 40_dwh_incremental_load.sql (инкрементная загрузка DWH)
|   +-- 60_restore_branch_from_dwh.sql (восстановление данных)
|
+-- docs/
|   +-- img/                     (диаграммы и графики)
|       +-- data_model.png       (ER-диаграмма филиалов)
|       +-- kimball.png          (звёздная схема DWH)
|       +-- view.png             (витрины данных)
|
+-- scripts/                     (утилиты)
    +-- restore_branch_from_dwh_cli.sh
    +-- test_restore_lab6.sh
```
...
**Surrogate Key**: Искусственный уникальный идентификатор (SERIAL), создаваемый БД.
Используется для связей в DWH вместо натуральных ключей.

**Natural Key**: Уникальный идентификатор из реальных данных
(customer_id из филиала, product_name и т.д.).

**Fact Table**: Таблица фактов, содержит измеримые события (продажи, заказы).
Строки - это транзакции, столбцы - это метрики.

**Dimension Table**: Таблица измерения, содержит справочные данные
(кто, что, когда, где). Справочник для контекста фактов.

**Star Schema**: Архитектура с одной таблицей фактов в центре и измерениями вокруг.
Похожа на звезду.

**FDW (Foreign Data Wrapper)**: Механизм в PostgreSQL для запросов удалённых БД
как если бы это были локальные таблицы.

**ETL**: Extract (извлечение), Transform (трансформация), Load (загрузка).
Процесс переноса данных из источников в хранилище.

**Idempotent**: Свойство операции, при котором повторное выполнение даёт тот же результат.
Безопасно запускать несколько раз.

**CDC (Change Data Capture)**: Механизм обнаружения изменений в исходных системах.
Используется для инкрементной загрузки.

**UPSERT**: Операция "вставить или обновить" (INSERT ... ON CONFLICT).

**SCD (Slowly Changing Dimension)**: Методология управления историей измерений.
Type 1 = перезаписать, Type 2 = добавить новую версию.

**Append-only**: Таблица, где добавляются только новые строки, старые не обновляются.
Сохраняет полную историю.

---

**Дата обновления**: Февраль 2026
**Статус**: Готов к использованию в качестве учебного примера архитектуры DWH
